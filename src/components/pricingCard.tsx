...
export default function PricingCard({ title, description, price, features, popular }: PricingCardProps) {
    return (
        <motion.div 
            ...
            className={`flex flex-col h-full md:max-w-sm max-w-full rounded-lg shadow-lg ${popular ? 'border-2 border-text-primary [box-shadow:0px_-50px_80px_-20px_rgba(255,255,255,0.025)_inset]  drop-shadow-[0_4px_20px_rgba(255,255,255,0.05)]' : 'border border-border'} relative font-jetbrains text-text-primary`}>
                {popular && (
